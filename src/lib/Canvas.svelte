...
//EVENT LISTENERS
//
window.addEventListener('resize', () => {template.width = parent.clientWidth;
  template.height = parent.clientHeight;console.log("width is ", template.width, " and height is ", template.height);drawDots(); drawAll(boxArray);clearButtons();drawMenu(); });
...
template.addEventListener('mouseup', e => {
  ...
  if (e.offsetX < 200){
    ...
    (buttonStore[i].y + buttonStore[i].height + 6 )) {
          ...
          //boxArray.push(temp); 
          //drawAll(boxArray); 
          canvasUtility.createChild(temp.type, temp.type, 'index', temp)
